...
object RuleBaseGenerator extends App {
  ...
  if (args.length < 2) {
    println("This application needs 3 parameters to be run:\n " +
      ...
      "(2) path of directory containing keys files and rules, may contain:\n" +
  ...
  val files_path = args(1)

  //creation of RB
  val all_keys = IOManager.computeAllKeys(input_directory_path)
  IOManager.writeKeys(files_path + all_keys_file, all_keys)

  val seen_keys_path = files_path + seen_keys_file
  val rule_list_path = files_path + rules_list_file
