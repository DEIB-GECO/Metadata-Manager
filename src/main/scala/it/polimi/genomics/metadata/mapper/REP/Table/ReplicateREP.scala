...
import it.polimi.genomics.metadata.mapper.{Replicate, Table}

import scala.collection.mutable.ListBuffer


class ReplicateREP(repTableId: REPTableId) extends REPTable(repTableId) with Replicate {
  ...
  override def setParameter(param: String, dest: String, insertMethod: (String, String) => String): Unit = {
    dest.toUpperCase match {
      case "SOURCEID" => {
        this.sourceIdList += insertMethod(param, param)
      }
      case "BIOREPLICATENUM" => {
        this.bioReplicateNumList += param.toInt
      }
      case "TECHREPLICATENUM" => {
        this.techReplicateNumList += param.toInt
      }
  ...
  override def getReplicateIdList(): List[Int] = {
    bioSampleIdList.toList
  }
